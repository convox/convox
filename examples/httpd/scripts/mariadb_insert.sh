...
mariadb -h${host} -P${port} -u${user} -p${pass} -D${dbname} -e 'CREATE TABLE IF NOT EXISTS data ( id int4 );'

counter=1
until [ $counter -gt 5 ]
do
  mariadb -h${host} -P${port} -u${user} -p${pass} -D${dbname} -e 'INSERT INTO data(id) VALUES (1);'
  ((counter++))
done
