...
| **nlb_security_group**  |                | The ID of an existing security group to attach to the rack's internet-facing router load balancer. Leave it blank and the rack creates and manages one, allowing inbound traffic from the CIDRs in **whitelist**. Requires a router load balancer created on 3.18.0 or later. Be cautious about this parameter, you might lose access to services by using improper security group |
