...
| **ami_id**               |                        | You can specify a custom Amazon Machine Image (AMI) ID to use for your Convox Rack nodes. Warning: using a custom AMI is considered an advanced, manual configuration and **is not officially recommended by Convox**. An invalid or incomplete AMI will break your Rack functionality. When Convox updates Kubernetes versions, custom AMI nodes will not receive automatic updates. You must manually update custom AMIs to maintain compatibility with newer Kubernetes versions. This configuration should only be used by users who fully understand the AWS EC2 and Kubernetes node requirements. **Use this feature at your own risk.** Here is the example guide to create eks custom ami: https://docs.aws.amazon.com/eks/latest/userguide/eks-ami-build-scripts.html|
| **availability_zones**   |                        | Specify a list of AZ names (minimum 3) to override the random automatic selection from AWS                     |
| **build_ami_id**         |                        | You can specify a custom Amazon Machine Image (AMI) ID to use for your Convox Rack nodes. Warning: using a custom AMI is considered an advanced, manual configuration and **is not officially recommended by Convox**. An invalid or incomplete AMI will break your Rack functionality. When Convox updates Kubernetes versions, custom AMI nodes will not receive automatic updates. You must manually update custom AMIs to maintain compatibility with newer Kubernetes versions. This configuration should only be used by users who fully understand the AWS EC2 and Kubernetes node requirements. **Use this feature at your own risk.**. Here is the example guide to create eks custom ami: https://docs.aws.amazon.com/eks/latest/userguide/eks-ami-build-scripts.html |
