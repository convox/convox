...
Convox 3.24 upgrades Kubernetes to 1.34, introduces the [`convox deploy-debug`](/reference/cli/deploy-debug) command, adds mixed ARM/x86 architecture support, and adds [Karpenter](/configuration/scaling/karpenter) as an opt-in alternative to Cluster Autoscaler for AWS EKS node provisioning. Later patch releases add [KEDA autoscaling](/configuration/scaling/keda) with scale-to-zero, [cost tracking](/management/cost-tracking) with [monthly budget caps](/management/budget-caps), [GPU observability](/observability/gpu-metrics), the [Contour ingress router](/configuration/ingress-router), and a set of security hardening parameters covering [Pod Security Admission](/configuration/rack-parameters/aws/pod_security_standard), [IMDS restrictions](/configuration/rack-parameters/aws/pod_imds_block_enabled), and the [Bottlerocket node OS](/configuration/rack-parameters/aws/karpenter_node_os).
