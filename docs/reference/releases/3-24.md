---
title: "3.24 Releases"
description: "Convox 3.24 upgrades Kubernetes to 1.34, adds the convox deploy-debug command, mixed ARM/x86 support, and Karpenter as an opt-in node provisioner for AWS EKS."
slug: 3-24-releases
url: /reference/releases/3-24
---
# 3.24 Releases

Convox 3.24 upgrades Kubernetes to 1.34, introduces the `convox deploy-debug` command, adds mixed ARM/x86 architecture support, and adds Karpenter as an opt-in alternative to Cluster Autoscaler for AWS EKS node provisioning. The 3.24.11 release refines Pod Security Admission transitions by routing Builds around App namespaces that retain an `enforce` label. The 3.24.10 release is a security hardening release adding an opt-in Bottlerocket node OS for Karpenter nodes, IMDS restrictions for App pods and build nodes, Kubernetes Pod Security Admission for App namespaces, a default seccomp profile and read-only root filesystems for Convox system containers, immutable ECR image tags, and a least-privilege IAM role for build nodes. The 3.24.9 release updates the Go toolchain and hardens archive extraction. The 3.24.8 release adds Contour (Envoy) as an opt-in alternative to the nginx ingress router on AWS, plus an automatic deploy-recovery fix for Services carrying a duplicate port. The 3.24.7 release adds ECR IAM policy customization, EKS Access Entries for migrating off the `aws-auth` ConfigMap, Karpenter enablement validation guards, and CloudWatch log streaming rate limiting. The 3.24.6 release adds KEDA-based autoscaling with scale-to-zero, per-app cost tracking and monthly budget caps, GPU observability with DCGM and Prometheus, HMAC webhook signing, and prebuilt image imports. Earlier releases include Fluentd memory tuning, Terraform timeout control, automatic parameter reconciliation across version transitions, and several reliability fixes.
...
- Added support for mixed ARM/x86 architecture node groups within a single rack with architecture-aware build scheduling via the `BuildArch` app parameter. The `BuildArch` parameter did not take effect until 3.25.3; on racks before that version the value is accepted and discarded
...
- Added `router_type` rack parameter for selecting the AWS rack ingress router. `nginx` (the default) is unchanged; `contour` switches the rack to a Contour (Envoy) ingress. Switching `router_type` on a rack with running apps takes every app offline until each one is redeployed, so it is currently intended for new racks or staging. See [Ingress Router](/configuration/ingress-router) for the full migration caveat
...
[View on GitHub](https://github.com/convox/convox/releases/tag/3.24.8)
...
- Hardened archive extraction across builds, `convox cp`, and App and Build imports. Each archive entry is verified to resolve inside its destination directory, and extraction is bounded in size
- Upgraded the build toolchain to Go 1.25.11 and refreshed security-sensitive dependencies (`golang.org/x/crypto`, `golang.org/x/net`, `github.com/moby/spdystream`)

**Fixes**

- Bounded log-stream retries when a CloudWatch log group is not present. A log tail now ends cleanly after about a minute instead of retrying indefinitely, most relevant on racks that ship logs externally with `fluentd_disable=true`

[View on GitHub](https://github.com/convox/convox/releases/tag/3.24.9)
...
- Added the [`karpenter_node_os`](/configuration/rack-parameters/aws/karpenter_node_os) rack parameter to run Karpenter workload nodes on the EKS-optimized Bottlerocket OS, with the required two-volume disk layout applied automatically
- Added the [`ecr_immutable_tags_enabled`](/configuration/rack-parameters/aws/ecr_immutable_tags_enabled) rack parameter to create App ECR repositories with immutable image tags
- Added the [`pod_imds_block_enabled`](/configuration/rack-parameters/aws/pod_imds_block_enabled) rack parameter to block App pod access to the EC2 instance metadata service (IMDS)
- Added the [`karpenter_build_imds_tokens`](/configuration/rack-parameters/aws/karpenter_build_imds_tokens) and [`karpenter_build_imds_hop_limit`](/configuration/rack-parameters/aws/karpenter_build_imds_hop_limit) rack parameters to configure IMDS options on the Karpenter build nodepool independently of the rack-wide settings
- Added the [`build_node_minimal_role_enabled`](/configuration/rack-parameters/aws/build_node_minimal_role_enabled) rack parameter that gives Karpenter build nodes a dedicated least-privilege IAM role
- Added the [`pod_security_standard`](/configuration/rack-parameters/aws/pod_security_standard) and [`pod_security_mode`](/configuration/rack-parameters/aws/pod_security_mode) rack parameters to apply a Kubernetes Pod Security Admission standard to App namespaces on AWS racks
- Added the [`seccomp_default_enabled`](/configuration/rack-parameters/aws/seccomp_default_enabled) rack parameter to apply the RuntimeDefault seccomp profile to Convox system pods on AWS racks
- Added the [`system_readonly_rootfs_enabled`](/configuration/rack-parameters/aws/system_readonly_rootfs_enabled) rack parameter to run the Convox system containers with a read-only root filesystem on AWS racks

**Updates**

- Added a TYPE column to `convox instances` showing each node's instance type
- Hardened telemetry manifest generation and refreshed the generated telemetry manifests

[View on GitHub](https://github.com/convox/convox/releases/tag/3.24.10)
...
- Builds now route to the dedicated per-App build namespace whenever an App namespace still carries a Pod Security Admission `enforce` label, including the transition window after leaving `enforce` mode, and build log streaming follows the build pod wherever it runs

[View on GitHub](https://github.com/convox/convox/releases/tag/3.24.11)
...
- [BuildArch](/configuration/app-parameters/aws/BuildArch) for architecture-aware build scheduling, which takes effect on racks running 3.25.3 or later
...
- [Webhooks](/configuration/webhooks#signing) for the `Convox-Signature` header reference (format `t=<unix-ts>,v1=<hex1>[,v1=<hex2>]`; multiple `v1=` segments may appear during key rotation, and receivers verify against any one)
...
- [karpenter_node_os](/configuration/rack-parameters/aws/karpenter_node_os) for running Karpenter workload nodes on Bottlerocket
- [ecr_immutable_tags_enabled](/configuration/rack-parameters/aws/ecr_immutable_tags_enabled) for immutable App ECR image tags
- [pod_imds_block_enabled](/configuration/rack-parameters/aws/pod_imds_block_enabled) for blocking App pod IMDS access
- [karpenter_build_imds_tokens](/configuration/rack-parameters/aws/karpenter_build_imds_tokens) and [karpenter_build_imds_hop_limit](/configuration/rack-parameters/aws/karpenter_build_imds_hop_limit) for build nodepool IMDS configuration
- [build_node_minimal_role_enabled](/configuration/rack-parameters/aws/build_node_minimal_role_enabled) for the dedicated build node IAM role
- [pod_security_standard](/configuration/rack-parameters/aws/pod_security_standard) and [pod_security_mode](/configuration/rack-parameters/aws/pod_security_mode) for Pod Security Admission on App namespaces
- [seccomp_default_enabled](/configuration/rack-parameters/aws/seccomp_default_enabled) for the RuntimeDefault seccomp profile on system pods
- [system_readonly_rootfs_enabled](/configuration/rack-parameters/aws/system_readonly_rootfs_enabled) for read-only system container root filesystems
