...
|---|---|---|
| Monthly Budget Cap | Hard cap on monthly spend in USD | $0.01 -- $100,000 |
| Alert Threshold | Percentage of cap at which alert notifications fire | 1% -- 100% (default: 80%) |
| At-Cap Action | Enforcement behavior when the cap is reached | See below |
| Pricing Adjustment | Multiplier applied to recorded spend to match actual cloud invoices | 0.1 -- 1.5 (default: 1.0) |
...
All mutations are audit-logged with the acting user's email.
...
Both actions use optimistic updates for responsive feedback and revert on failure.
...
- **Restore Now** -- Immediately restores all Services to their original replica counts and applies a 24-hour cooldown before auto-shutdown can re-arm (prevents rapid flap cycles)
...
- [Deploy Wizard](/console/deploy-wizard)
