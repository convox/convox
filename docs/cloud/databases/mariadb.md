...
Convox Cloud provides fully managed MariaDB databases through AWS RDS. These are Console-managed Cloud Databases on Convox-hosted infrastructure, not database resources on a self-managed Rack. See [Cloud Databases](/cloud/databases) for an overview and [Cloud vs Rack Comparison](/cloud/comparison) for the differences.
