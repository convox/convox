...
On Azure, spend is priced against eastus Linux list prices, keyed by the ARM SKU name reported on each node (for example `Standard_D4s_v5`). Spot node pools are detected from the `kubernetes.azure.com/priority` node label and discounted automatically.
...
For racks with a single small workload node, enabling Console monitoring on top of cost tracking can overcommit the node and trigger a kubelet failure that drags pods into a stuck Terminating state. Recommended minimums for clusters that intend to surface cost data through the Console:
- One workload node of `Standard_D2s_v5` or larger (or any 2 vCPU / 4+ GiB size), OR
- Two or more workload nodes of any size where the user-workload pods can spread off the prometheus statefulset's node, OR
- Autoscaling headroom on the default node pool ([max_on_demand_count](/configuration/rack-parameters/azure/max_on_demand_count) above current usage) so the cluster can grow to absorb chart-install spikes.
...
- The pricing table is updated per Convox release. Azure prices are eastus Linux consumption list prices; pin a custom multiplier (e.g., to model a committed-use discount or a different region's rates) via the per-app `pricing-adjustment` budget option (`convox budget set --pricing-adjustment 0.7 <app>`).
- Spot capacity-type discount is applied automatically (a default factor of `0.30`, with per-instance factors for GPU SKUs) when the pod runs on an AKS spot node pool. Per-instance overrides are configurable via the rack's pricing table.
