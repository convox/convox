...
## Description
The `prometheus_url` parameter overrides the rack's auto-resolved Prometheus endpoint with a customer-supplied URL. The rack-API queries this endpoint on each `convox ps` / `convox services` invocation to enrich responses with `gpu-util`, `gpu-mem-used`, `gpu-mem-total` per-pod fields.
...
Set this parameter ONLY when you have an external Prometheus aggregator (Grafana Cloud Prometheus, AWS AMP, or a self-hosted federation hub) and want the rack to query that aggregator INSTEAD. **Customer-set value always wins** over the auto-resolution chain.
...
- The auto-resolution priority is: customer-set `prometheus_url` > paid metered metrics Prometheus > free-tier GPU Prometheus (`gpu_observability_enable`) > none. Whether the DCGM exporter runs inside the rack is controlled by `gpu_observability_enable`; this parameter only overrides the QUERY-side endpoint.
