...
description: "The prometheus_url AWS rack parameter sets the external Prometheus URL for KEDA autoscale triggers and the Console GPU columns, defaulting to empty."
slug: prometheus_url
...
## Description
External Prometheus URL for KEDA autoscale triggers and observability. Setting this value adds GPU telemetry to the process-list response the rack API returns, which the Convox Console renders as the GPU columns in the service detail pod list. When empty (default), those columns show a dash even when a Prometheus chart is installed via Console.
...
## Default Value
The default is `""` (empty string). When empty, the rack skips the GPU query: there is no error, and the GPU columns show a dash instead of a value.

## Use Cases
- **Convox-Console-managed users**: must explicitly set this to surface the GPU columns in the Console. Use the in-cluster service URL: paid → `http://convox-kube-prometheus-sta-prometheus.convox-monitoring.svc.cluster.local:9090`, free → `http://prometheus-gpu-metrics-server.kube-system.svc.cluster.local:80`.
...
Updating parameters... OK
```
Note: clearing `prometheus_url` removes KEDA autoscale based on Prometheus metrics and blanks the GPU columns until the parameter is re-set. The rack does not auto-resolve a URL; empty means empty.
...
- The rack uses a 5-second timeout per query so a misconfigured or unreachable endpoint cannot stall the process list indefinitely. On a query timeout, the response is returned without GPU fields populated and the Console shows a dash rather than an error.
