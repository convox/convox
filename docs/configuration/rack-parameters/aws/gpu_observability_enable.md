...
**Resource overhead per GPU rack:** the DCGM exporter itself is modest, at 100m CPU request / 200m limit and 128Mi memory request / 512Mi limit. The Console-deployed Prometheus chart adds its own footprint (sized at chart-time per plan).
...
- A GCP equivalent of this parameter exists as of rack version `3.25.3`, see [gpu_observability_enable (GCP)](/configuration/rack-parameters/gcp/gpu_observability_enable). What remains AWS-only is the Convox-managed Prometheus and the Console GPU dashboard chain. Convox supports NVIDIA GPU instance families beginning with `g` or `p`, for example g5, g6, p4, or p5, on EKS only at this time.
...
- [prometheus_url](/configuration/rack-parameters/aws/prometheus_url): Set the Prometheus URL for KEDA autoscale and the Console GPU columns.
