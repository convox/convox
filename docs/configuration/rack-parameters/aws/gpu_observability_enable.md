...
- A GCP equivalent of this parameter exists as of rack version `3.25.3`, see [gpu_observability_enable (GCP)](/configuration/rack-parameters/gcp/gpu_observability_enable). What remains AWS-only is the Convox-managed Prometheus and the Console GPU dashboard chain. The Convox-supported NVIDIA GPU instance families (P3, P4, G4, G5) are EKS-only at this time.
