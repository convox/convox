...
func (s *Server) setupRoutes(r stdapi.Router) {
	r.Use(s.Authorize)
	r.Use(s.RejectReservedApp)

	r.Route("POST", "/apps/{app}/cancel", s.AppCancel)
	r.Route("POST", "/apps", s.AppCreate)
	r.Route("DELETE", "/apps/{app}", s.AppDelete)
	r.Route("GET", "/apps/{app}/diagnose", s.AppDiagnose)
	r.Route("GET", "/apps/{app}", s.AppGet)
	r.Route("GET", "/apps", s.AppList)
	r.Route("SOCKET", "/apps/{app}/logs", s.AppLogs)
	r.Route("GET", "/apps/{app}/manifest/services/{service}", s.AppManifestService)
	r.Route("GET", "/apps/{app}/metrics", s.AppMetrics)
	r.Route("GET", "/apps/{app}/metrics-by-service", s.MetricsByService)
	r.Route("PUT", "/apps/{app}", s.AppUpdate)
	r.Route("GET", "/apps/{app}/budget", s.AppBudgetGet)
	r.Route("POST", "/apps/{app}/budget", s.AppBudgetSet)
	r.Route("DELETE", "/apps/{app}/budget", s.AppBudgetClear)
	r.Route("POST", "/apps/{app}/budget/reset", s.AppBudgetReset)
	r.Route("GET", "/apps/{app}/budget/shutdown-state", s.AppBudgetShutdownStateGet)
	r.Route("POST", "/apps/{app}/budget/simulate-shutdown", s.AppBudgetSimulate)
	...
	r.Route("GET", "/apps/{app}/services/{service}/metrics", s.ServiceMetrics)
	r.Route("POST", "/apps/{app}/services/{name}/restart", s.ServiceRestart)
	r.Route("POST", "/apps/{app}/services/{service}/scale-override", s.ServiceScaleOverrideSet)
	r.Route("POST", "/apps/{app}/services/{service}/triggers/disable", s.ServiceTriggersDisable)
	r.Route("POST", "/apps/{app}/services/{service}/triggers/enable", s.ServiceTriggersEnable)
	r.Route("POST", "/apps/{app}/services/{service}/triggers/threshold", s.ServiceTriggersThresholdSet)
