...
func Instances(rack sdk.Interface, c *stdcli.Context) error {
	...
	t := c.Table("ID", "STATUS", "STARTED", "PS", "CPU", "MEM", "PUBLIC", "PRIVATE", "TYPE")
	...
	for _, i := range is {
		...
		t.AddRow(i.Id, i.Status, common.Ago(i.Started), fmt.Sprintf("%d", i.Processes), cpuPercent, memPercent, i.PublicIp, i.PrivateIp, i.InstanceType)
	}
