...
func (p *Provider) dockerConfigLoad(secret string) (*dockerConfig, error) {
	s, err := p.Cluster.CoreV1().Secrets(p.Namespace).Get(context.Background(), secret, am.GetOptions{})
...
func (p *Provider) dockerConfigSave(secret string, dc *dockerConfig) error {
	...
	s, err := p.Cluster.CoreV1().Secrets(p.Namespace).Get(context.Background(), secret, am.GetOptions{})
	if ae.IsNotFound(err) {
		_, err := p.Cluster.CoreV1().Secrets(p.Namespace).Create(context.Background(), &ac.Secret{
	...
	_, err = p.Cluster.CoreV1().Secrets(p.Namespace).Update(context.Background(), s, am.UpdateOptions{})
	if err != nil {
		return errors.WithStack(err)
	}
