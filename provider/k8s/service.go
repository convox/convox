...
func (p *Provider) ServiceList(app string) (structs.Services, error) {
	...
	dss, err := p.Cluster.AppsV1().DaemonSets(p.AppNamespace(app)).List(context.Background(), lopts)
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (p *Provider) serviceRestartDaemonset(app, name string) error {
	...
	s, err := ds.Get(context.Background(), name, am.GetOptions{})
	if err != nil {
		return errors.WithStack(err)
	}
	...
	if _, err := ds.Update(context.Background(), s, am.UpdateOptions{}); err != nil {
		return errors.WithStack(err)
	}
...
func (p *Provider) ServiceUpdate(app, name string, opts structs.ServiceUpdateOptions) error {
	d, err := p.Cluster.AppsV1().Deployments(p.AppNamespace(app)).Get(context.Background(), name, am.GetOptions{})
	if err != nil {
		return errors.WithStack(err)
	}
	...
	if _, err := p.Cluster.AppsV1().Deployments(p.AppNamespace(app)).Update(context.Background(), d, am.UpdateOptions{}); err != nil {
		return errors.WithStack(err)
	}
