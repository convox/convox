...
func (p *Provider) ReleasePromote(app, id string, opts structs.ReleasePromoteOptions) error {
	...
	if acquired, release := tryAcquireWatchSlot(app, id); watchable && acquired {
		s := state // own a heap copy so the goroutine doesn't alias
		// The request context is cancelled as soon as this handler returns, and
		// ListDeploymentsFromInformer binds its live fallback to the receiver's.
		wp := *p
		wp.ctx = context.WithoutCancel(p.ctx)
		go wp.runReleasePromoteWatcher(wp.ctx, app, &s, release)
	} else if acquired {
		release()
	}
