...
func TestSystemResourceCreate(t *testing.T) {
	testProvider(t, func(p *k8s.Provider) {
		...
		cm, err := fc.CoreV1().ConfigMaps(p.Namespace).Get(context.TODO(), "webhooks", am.GetOptions{})
		require.NoError(t, err)
		require.NotNil(t, cm)
...
func TestSystemResourceDelete(t *testing.T) {
	testProvider(t, func(p *k8s.Provider) {
		...
		cm2, err := fc.CoreV1().ConfigMaps(p.Namespace).Get(context.TODO(), "webhooks", am.GetOptions{})
		require.NoError(t, err)
