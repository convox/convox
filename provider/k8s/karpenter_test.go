...
func TestKarpenterCleanup_DeletesOrphanedNodes(t *testing.T) {
	// Karpenter node objects with label karpenter.sh/nodepool should be deleted
	// after CRD cleanup to prevent NotReady nodes blocking EKS addon health checks.
	...
	p := newKarpenterTestProvider(t,
		[]runtime.Object{karpenterNode, eksNode},
		[]runtime.Object{np},
	)
	...
	require.Equal(t, "ip-10-1-116-97", allNodes.Items[0].Name)
}
