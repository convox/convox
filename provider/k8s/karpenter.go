...
func (p *Provider) KarpenterCleanup() error {
	...
	// Final step: terminate Karpenter EC2 instances and delete orphaned node
	// objects and pods. This is critical — without EC2 termination, kubelet
	// re-registers nodes, daemonset pods get scheduled on NotReady nodes, and
	// EKS addon health checks block Terraform indefinitely.
	if err := p.deleteKarpenterNodes(); err != nil {
		return fmt.Errorf("KarpenterCleanup: %w", err)
	}
...
// deleteKarpenterNodes terminates Karpenter EC2 instances, then removes their
// K8s node objects and orphaned pods. EC2 termination is the critical step —
// without it, kubelet keeps re-registering nodes and the daemonset controller
// creates Pending pods that block EKS addon health checks, hanging Terraform.
func (p *Provider) deleteKarpenterNodes() error {
	ctx := context.TODO()
	...
	if err != nil {
		return fmt.Errorf("list karpenter nodes: %w", err)
	}
	if len(nodes.Items) == 0 {
		fmt.Println("KarpenterCleanup: no Karpenter node objects to clean up")
		return nil
	}

	// Collect node names and extract EC2 instance IDs from providerID
	karpenterNodeNames := map[string]bool{}
	var instanceIDs []string
	for _, node := range nodes.Items {
		karpenterNodeNames[node.Name] = true
		if id := instanceIDFromProviderID(node.Spec.ProviderID); id != "" {
			instanceIDs = append(instanceIDs, id)
		}
	}

	fmt.Printf("KarpenterCleanup: found %d Karpenter nodes, %d EC2 instance IDs\n", len(nodes.Items), len(instanceIDs))

	// Cordon nodes — prevents regular (non-daemonset) pod scheduling.
	// DaemonSet pods still tolerate unschedulable, but cordoning reduces churn.
	for _, node := range nodes.Items {
		patch := []byte(`{"spec":{"unschedulable":true}}`)
		if _, err := p.Cluster.CoreV1().Nodes().Patch(ctx, node.Name, types.MergePatchType, patch, am.PatchOptions{}); err != nil {
			fmt.Printf("KarpenterCleanup WARNING: failed to cordon %s: %v\n", node.Name, err)
		}
	}

	// Terminate EC2 instances — the critical fix.
	// After Phase 2 killed the Karpenter controller, nobody else will terminate
	// these instances. Without this, kubelet keeps running, re-registers nodes,
	// daemonset controller creates Pending pods on NotReady nodes, and EKS addon
	// health checks block Terraform indefinitely.
	if len(instanceIDs) > 0 {
		if p.TerminateKarpenterEC2 != nil {
			if err := p.TerminateKarpenterEC2(instanceIDs); err != nil {
				fmt.Printf("KarpenterCleanup WARNING: EC2 termination failed: %v (continuing with node/pod cleanup)\n", err)
			}
		} else {
			fmt.Println("KarpenterCleanup WARNING: EC2 termination not available (no AWS provider), nodes may re-register")
		}
	}

	// Cleanup loop: delete nodes first, then pods, until both are clean.
	// Order matters: deleting the node first means the DaemonSet controller
	// removes it from its desired set, so pod recreations after our pod
	// deletion are orphaned (bound to non-existent node) rather than
	// scheduled. The next pass catches any stragglers.
	deadline := time.Now().Add(120 * time.Second)
	for pass := 1; time.Now().Before(deadline); pass++ {
		// Delete any Karpenter nodes
		nodes, _ := p.Cluster.CoreV1().Nodes().List(ctx, am.ListOptions{
			LabelSelector: "karpenter.sh/nodepool",
		})
		if nodes != nil {
			for _, node := range nodes.Items {
				karpenterNodeNames[node.Name] = true
				p.Cluster.CoreV1().Nodes().Delete(ctx, node.Name, am.DeleteOptions{})
			}
		}

		// Delete orphaned pods on former Karpenter nodes
		orphanedPods := p.deletePodsOnNodes(ctx, karpenterNodeNames)

		noNodes := nodes == nil || len(nodes.Items) == 0
		if noNodes && orphanedPods == 0 {
			fmt.Printf("KarpenterCleanup: nodes and pods clean after %d passes\n", pass)
			break
		}
		...
		if pass%3 == 0 {
			...
			fmt.Printf("KarpenterCleanup: pass %d — %d nodes, %d orphaned pods\n", pass, nodeCount, orphanedPods)
		}
	...
	if nodes != nil && len(nodes.Items) > 0 {
		fmt.Printf("KarpenterCleanup WARNING: %d Karpenter nodes still present after cleanup\n", len(nodes.Items))
		return fmt.Errorf("%d Karpenter nodes still present after cleanup", len(nodes.Items))
	}

	fmt.Println("KarpenterCleanup: all Karpenter nodes and orphaned pods removed")
	return nil
}

// deletePodsOnNodes force-deletes all kube-system pods bound to the given node names.
// Returns the number of pods deleted.
func (p *Provider) deletePodsOnNodes(ctx context.Context, nodeNames map[string]bool) int {
	pods, err := p.Cluster.CoreV1().Pods("kube-system").List(ctx, am.ListOptions{})
	if err != nil {
		fmt.Printf("KarpenterCleanup WARNING: failed to list pods: %v\n", err)
		return 0
	}
	...
	for _, pod := range pods.Items {
		if nodeNames[pod.Spec.NodeName] {
			if err := p.Cluster.CoreV1().Pods("kube-system").Delete(ctx, pod.Name, am.DeleteOptions{
				GracePeriodSeconds: &gracePeriod,
			}); err == nil {
				deleted++
			}
...
// node's spec.providerID. Format: aws:///ZONE/INSTANCE-ID
