...
func (p *Provider) KarpenterCleanup() error {
	...
	// Final step: delete orphaned Karpenter node objects from the cluster.
	// The EC2 instances are terminated (by the controller's graceful drain or EC2
	// safety net), but the K8s node objects linger as NotReady. Daemonset pods
	// stuck on these NotReady nodes block EKS addon health checks (e.g., EBS CSI),
	// which blocks the Terraform apply.
	if err := p.deleteKarpenterNodes(); err != nil {
		fmt.Printf("KarpenterCleanup WARNING: failed to delete Karpenter node objects: %v\n", err)
	}
...
// deleteKarpenterNodes removes Karpenter node objects and any orphaned pods
// stuck on them. This prevents EKS addon health checks (EBS CSI, VPC CNI) from
// blocking Terraform due to Pending daemonset pods on dead nodes.
//
// The cleanup is thorough: delete nodes, wait for EC2 termination, re-delete
// any kubelet re-registrations, then force-delete any remaining pods that are
// stuck on Karpenter nodes (Pending or on NotReady nodes).
func (p *Provider) deleteKarpenterNodes() error {
	ctx := context.TODO()

	// Pass 1: delete all Karpenter nodes
	...
	// Collect node names for pod cleanup later
	karpenterNodeNames := map[string]bool{}
	for _, node := range nodes.Items {
		karpenterNodeNames[node.Name] = true
		fmt.Printf("KarpenterCleanup: deleting node %s\n", node.Name)
		p.Cluster.CoreV1().Nodes().Delete(ctx, node.Name, am.DeleteOptions{})
	}

	// Wait for EC2 instances to fully terminate (kubelet dies, stops re-registering)
	fmt.Println("KarpenterCleanup: waiting 30s for EC2 instances to fully terminate...")
	time.Sleep(30 * time.Second)

	// Pass 2: catch any kubelet re-registrations
	nodes, _ = p.Cluster.CoreV1().Nodes().List(ctx, am.ListOptions{
		LabelSelector: "karpenter.sh/nodepool",
	})
	if nodes != nil && len(nodes.Items) > 0 {
		fmt.Printf("KarpenterCleanup: %d nodes re-registered, deleting again\n", len(nodes.Items))
		for _, node := range nodes.Items {
			karpenterNodeNames[node.Name] = true
			p.Cluster.CoreV1().Nodes().Delete(ctx, node.Name, am.DeleteOptions{})
		}
		// Second wait — shorter, EC2 should definitely be gone by now
	...
	// Final safety net: force-delete any pods in kube-system that are stuck on
	// Karpenter nodes. This directly eliminates the condition that causes EKS
	// addon health checks to hang — even if a node object somehow persists,
	// removing the stuck pods means the addon reports healthy.
	// Run twice with a gap to catch daemonset controller recreations.
	for podPass := 1; podPass <= 2; podPass++ {
		if podPass == 2 {
			time.Sleep(10 * time.Second)
		}
		fmt.Printf("KarpenterCleanup: cleaning up orphaned pods on former Karpenter nodes (pass %d/2)\n", podPass)
		pods, err := p.Cluster.CoreV1().Pods("kube-system").List(ctx, am.ListOptions{})
		if err != nil {
		...
		deleted := 0
		for _, pod := range pods.Items {
			if karpenterNodeNames[pod.Spec.NodeName] {
				fmt.Printf("KarpenterCleanup: force-deleting orphaned pod %s (node %s)\n", pod.Name, pod.Spec.NodeName)
				if err := p.Cluster.CoreV1().Pods("kube-system").Delete(ctx, pod.Name, am.DeleteOptions{
					GracePeriodSeconds: &gracePeriod,
				}); err != nil {
					fmt.Printf("KarpenterCleanup WARNING: failed to delete pod %s: %v\n", pod.Name, err)
				} else {
					deleted++
				}
			}
		...
		if deleted == 0 {
			fmt.Println("KarpenterCleanup: no orphaned pods found")
			break
		}
		fmt.Printf("KarpenterCleanup: deleted %d orphaned pods\n", deleted)
	}

	// Also do a final node deletion pass + verify
	nodes, _ = p.Cluster.CoreV1().Nodes().List(ctx, am.ListOptions{
		LabelSelector: "karpenter.sh/nodepool",
	})
	if nodes != nil && len(nodes.Items) > 0 {
		for _, node := range nodes.Items {
			fmt.Printf("KarpenterCleanup: final delete of node %s\n", node.Name)
			p.Cluster.CoreV1().Nodes().Delete(ctx, node.Name, am.DeleteOptions{})
		}
		fmt.Printf("KarpenterCleanup WARNING: %d Karpenter nodes required final cleanup\n", len(nodes.Items))
		return fmt.Errorf("%d Karpenter nodes required additional cleanup", len(nodes.Items))
	}
