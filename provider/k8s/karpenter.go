...
// deleteKarpenterNodes deletes K8s node objects that were provisioned by Karpenter.
// After EC2 instances are terminated, the node objects linger as NotReady with
// daemonset pods stuck Pending. This blocks EKS addon health checks and Terraform.
func (p *Provider) deleteKarpenterNodes() error {
	...
	for _, node := range nodes.Items {
		fmt.Printf("KarpenterCleanup: deleting node object %s\n", node.Name)
		if err := p.Cluster.CoreV1().Nodes().Delete(ctx, node.Name, am.DeleteOptions{}); err != nil {
			fmt.Printf("KarpenterCleanup WARNING: failed to delete node %s: %v\n", node.Name, err)
			failures++
		}
	}

	if failures > 0 {
		return fmt.Errorf("failed to delete %d node objects", failures)
	}

	fmt.Printf("KarpenterCleanup: deleted %d Karpenter node objects\n", len(nodes.Items))
	return nil
}
